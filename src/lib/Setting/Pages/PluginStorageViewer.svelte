<script lang="ts">
    ...
    // There is no ownership metadata tying a key to a plugin — see
    // .agent discussion. Edit/delete are allowed directly, guarded by confirm.
    ...
    async function load() {
        ...
        try {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                list.push({ key, raw, str, size: str.length * 2, type: detectType(str) })
                loadProgress = i + 1
...
<div class="flex flex-col gap-1 max-h-[60vh] overflow-y-auto rounded-md border border-darkborderc/50 p-1">
    ...
    {:else}
        {#each filtered as entry (entry.key)}
            ...
            >
                <span class="font-mono text-sm text-textcolor break-all flex-1 min-w-0">{entry.key}</span>
